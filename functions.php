...
function hackeryou_styles(){
	...
	wp_enqueue_style('googlefonts', 'https://fonts.googleapis.com/css?family=Crimson+Text');
...
function hackeryou_scripts() {
  ...
  wp_enqueue_script(
  	'gsap',
  	"http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://cdnjs.cloudflare.com/ajax/libs/gsap/1.20.2/TweenMax.min.js",
  	false, //dependencies
  	null, //version number
  	true //load in footer
  );
  ...
  wp_enqueue_script(
    ...
    array( 'jquery', 'gsap', 'plugins' ), //dependencies
    null, // version number
    true //load in footer
  );
}
